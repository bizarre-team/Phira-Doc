...
- ! 全新的谱面收藏夹功能：创建收藏夹以管理谱面，并通过云端同步或分享
- ! 更强大的本地谱面管理：支持批量导入/导出、手动重排序、搜索过滤等功能
- ! 新增多项玩法扩展模组：上隐、彩虹（颜色效果）、夜店（高倍速）、突然死亡、禁用着色器
- ! 新音频系统与高级配置：支持多类型输出、支持配置偏好采样率；更智能的输出设备决策
- 视频支持升级：支持 HEVC/H.265、av1、vp9 等视频格式
- UI微调（谱面浏览界面、加载界面与结算界面），提升整体视觉体验和操作流畅度
- 谱面支持升级：RPE 1.7.0 速度缓动（需手动开启）、父子线旋转、判定区域、音符颜色、视频绑线等
- 游戏内消息支持快捷跳转：支持跳转到相关谱面或链接
- 杂项新功能：减少动画效果、结算界面 Early/Late 显示、忘记密码入口、可选关闭 AP/FC 指示器
- 内部功能优化
...
修复：
- 优化视频支持：修复部分谱面解锁视频音频有误，以及部分视频无法播放的问题
- RPE UI绑定修复：新导入的谱面默认开启，旧谱面需手动启用
- 修复系列崩溃问题：本地谱面未完整删除，导入非法资源包等
- 资源包颜色支持：修复资源包不支持半透明颜色的问题

本地化：
- 新增/修复多语言翻译（zh-TW、vi-VN、ru-RU、fr-FR、th-TH、tr-TR 等），翻译键与措辞修正
